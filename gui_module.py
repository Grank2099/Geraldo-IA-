...
from voice_module import simplelistenoff, simplelistenon, get_voice_command_online, get_voice_command_offline
    ...
    def voice_command_detection_thread(self):
        while self.shared_variable['detect_voice']:
            ...
            if is_internet_connected_android():
                get_voice_command = get_voice_command_online
            else:
                get_voice_command = get_voice_command_offline
            while True:
                full_command = get_voice_command(trigger_word='geraldo', language='pt-BR')

                if full_command:
                    if "pesquisa por" in full_command:
                        # Extrair conteúdo do comando
                    ...
                    elif "liga para" in full_command:
                        # Extrair conteúdo do comando
                        contact_name = full_command.split("liga para", 1)[1].strip()
                        make_call(contact_name)
                        
                    elif "liga para o" in full_command:
                        # Extrair conteúdo do comando
                        contact_name = full_command.split("liga para o", 1)[1].strip()
                        make_call(contact_name)
                    ...
                    elif "estamos online" in full_command:
                        if is_internet_connected_android():
                            speak("Estamos conectados.")
                        else:
                            speak("Estamos sem conexão.")
                    #Saindo da log (apesar de acabar reiniciando sozinho)
